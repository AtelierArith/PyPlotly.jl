...
RUN pip3 install plotly numpy pandas sklearn
...
ENV JULIA_PROJECT "@."
WORKDIR /workspace/PyPlotly.jl

USER root
RUN chown -R ${NB_UID} /workspace/PyPlotly.jl
USER ${USER}

RUN mkdir -p /workspace/PyPlotly.jl/src && echo "module PyPlotly end" > /workspace/PyPlotly.jl/src/PyPlotly.jl
COPY ./Project.toml /workspace/PyPlotly.jl
